...
FROM    PIPEndorsement e WITH (NOLOCK)
        ...
        LEFT OUTER JOIN dbo.PEOPLE pu ON pu.PEOPLE_SKEY = pcpd.UnderwriterPeopleSKey
UNION ALL
/* BA submissions which are NOT in the PIP tables and have no endorsment - should not overlap with above */
SELECT DISTINCT
        pcpd.PIPCommonPolicyDeclarationId AS PolicyId ,
		...
		'' -- no address for PG
FROM    dbo.SUBMISSION AS s WITH (NOLOCK)
        ...
        LEFT OUTER JOIN dbo.ENDORSEMENT e ON e.SUB_RECORD_NUMBER = s.SUB_RECORD_NUMBER
        LEFt OUTER JOIN dbo.PIPCommonPolicyDeclaration pcpd ON s.SUB_RECORD_NUMBER = pcpd.SUB_RECORD_NUMBER
        LEFT OUTER JOIN dbo.INSURANCE_COMPANIES ic ON s.COMPANY_RECORD_NUMBER = ic.COMPANY_RECORD_NUMBER
        LEFT OUTER JOIN dbo.POLICY_SYMBOL ps ON s.SUB_POLICY_SYMBOL = ps.SYMBOL_SKEY
WHERE   1 = 1
        AND s.DEPARTMENT_NUMBER = 10006
        AND pcpd.SUB_RECORD_NUMBER IS NULL
        AND e.SUB_RECORD_NUMBER IS NULL
        --AND s.QB_SEQUENCE_NO > 0
        AND s.SUB_CURRENT_EXPIRATION_DATE > '1/1/2010'
UNION ALL
/* BA submissions are NOT in the PIP tables and DO have endorsements - should not overlap with above */
SELECT DISTINCT
        pcpd.PIPCommonPolicyDeclarationId AS PolicyId ,
        LTRIM(RTRIM(ic.COMPANY_CODE))
        + LTRIM(RTRIM(ps.POLICY_CODE))
...
FROM    dbo.ENDORSEMENT AS e WITH (NOLOCK)
