...
SELECT DISTINCT 
    ...
    ( CASE
		...
		WHEN EXISTS ( select 1
				FROM dbo.RuleBasedDocSubTypeXref rbdstx2
				WHERE rbdstx2.SubTypeId = 4004
				AND rbdstx2.DocId = rbd.DocId)				 
			THEN 'Professional Liability'
		WHEN EXISTS ( select 1
				FROM dbo.RuleBasedDocSubTypeXref rbdstx2
				WHERE rbdstx2.SubTypeId = 4005
				AND rbdstx2.DocId = rbd.DocId)				 
			THEN 'Inland Marine'
		WHEN EXISTS ( select 1
				FROM dbo.RuleBasedDocSubTypeXref rbdstx2
				WHERE rbdstx2.SubTypeId = 4006
				AND rbdstx2.DocId = rbd.DocId)				 
			THEN 'Liquor Liability'
		WHEN EXISTS ( select 1
				FROM dbo.RuleBasedDocSubTypeXref rbdstx2
				WHERE rbdstx2.SubTypeId = 4036
				AND rbdstx2.DocId = rbd.DocId)				 
			THEN 'Glass'
		ELSE
			'Undetermined'
